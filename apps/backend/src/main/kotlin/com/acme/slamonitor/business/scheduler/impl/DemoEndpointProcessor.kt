...
@Service
class DemoEndpointProcessor : EndpointProcessor {
    override suspend fun check(endpoint: EndpointView): EndpointResult {
        LOG.info("processing job ${endpoint.id}")
...
private val LOG by lazy { LoggerFactory.getLogger(DemoEndpointProcessor::class.java) }
